...
class ProjectService:
    ...
    def create_project(self, title:str, description:str) -> Project:
        ...
        with SessionLocal.begin() as session:
            if self.project_repo.count_all(session=session) >= int(os.getenv('MAX_NUMBER_OF_PROJECT', 0)):
                raise ValueError('max number of project exceeded')
            return self.project_repo.create(session=session, title=title, description=description)
    
    def update_project(self, project_code:int, title:str, description:str) -> Project:
        validate_project_title(title=title)
        validate_project_description(description=description)
        with SessionLocal.begin() as session:
            return self.project_repo.update(session=session, project_code=project_code, title=title, description=description)

    def delete_project(self, project_code:int) -> None:
        with SessionLocal.begin() as session:
            self.project_repo.delete(session=session, project_code=project_code)

    def list_projects(self) -> Sequence[Project]:
        with SessionLocal.begin() as session:
            return self.project_repo.all(session=session)
    
    def list_project_tasks(self, project_code:int) -> Sequence[Task]:
        with SessionLocal.begin() as session:
            project = self.project_repo.get(session=session, project_code=project_code)
            if not project:
                raise ValueError('invalid project_code')
            return self.task_repo.filter(session=session, project_code=project_code)